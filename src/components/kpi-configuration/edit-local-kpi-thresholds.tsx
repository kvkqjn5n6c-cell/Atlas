...
function validateThresholds(targetValue: number, warningThreshold: number, criticalThreshold: number) {
  const errors: string[] = [];
  ...
  if (warningThreshold > targetValue) {
    warnings.push("Le seuil de surveillance dépasse l'objectif. Vérifiez la logique métier du KPI.");
  }
...
export function EditLocalKpiThresholds({
  ...
  const validation = validateThresholds(targetValue, warningThreshold, criticalThreshold);
